...
def search(infoDict, depth, fullList):
    ...
    appendset = set()
    for sublist in fullList[depth + 1:]:
        for item in sublist:
            appendset.add(item)
    opList = ["<=", "=", "<"]
    for i in range(infoDict["argl"]):
        for j in range(infoDict["argl"]):
            if i == j:
                continue
            for op in opList:
                infoDict["cond"][depth].append([op, i, j])
                newcurset = Filter(infoDict["cond"][depth], curset)
                newapset = Filter(infoDict["cond"][depth], appendset)
                if len(curset) != len(newcurset) or len(appendset) == len(newapset):
                    infoDict["cond"][depth].pop(-1)
                elif len(newapset) == 0:
...
def convert2Sygus(infoDict):
    ...
    #print("\033[32m|—\033[33m candidates: \n \033[36m", candidates)
    #print("\033[32m|—\033[33m args: \n \033[36m", args)
    #print("\033[32m|—\033[33m conditions: \n \033[36m", conditions)
    ...
    def dfs(candidates, args, conditions, depth):
        ...
        return ["ite"] + [conditionStr] + [candi2arg(candidates[depth], args)] + [dfs(candidates, args, conditions, depth + 1)]

    curStr = dfs(candidates, args, conditions, depth=0)
    curStr = translator.toString(curStr)
    #print("\033[32m|—\033[33m curStr: \n \033[36m", curStr)
...
def Solver(bmExpr):
    ...
    #print("\033[32m|—\033[33m dic: \n \033[36m", dic)
    constraints = getConstraints(bmExpr)
  
    #print("\033[32m|—\033[33m constraints:\n \033[36m", constraints)
    varlist = getVarlist(bmExpr)

    #print("\033[32m|—\033[33m varlist: \n \033[36m", varlist)
    maxdepth = len(candidates) - 1
    boolExp = [[] for _ in range(maxdepth)]

    #print("\033[32m|—\033[33m boolExp: \n \033[36m", boolExp)
    depth = 0
    synth = getSynth(bmExpr)

    #print("\033[32m|—\033[33m synth: \n \033[36m", synth)
    funcname = synth[1]
    #print("\033[32m|—\033[33m Idx: \n \033[36m", "Idx" not in funcname)
    testcases = generateTestcase(varlist, withmax = "Idx" not in funcname)

    print("\033[32m|—\033[33m testcases: \n \033[36m", testcases)
    ...
    print("\033[32m|—\033[33m fullList: \n \033[36m", fullList)
    
    #print("\033[32m|—\033[33m infoDict: \n \033[36m", infoDict)
